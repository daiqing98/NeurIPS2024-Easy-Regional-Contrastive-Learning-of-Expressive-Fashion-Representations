...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        self.token_9_embedding = nn.Parameter(scale * torch.randn(width))
        self.token_10_embedding = nn.Parameter(scale * torch.randn(width))
        ...
        self.proj_s5 =  nn.Parameter(scale * torch.randn(width, output_dim))
    ...
    def select(self, x, num_tokens, select=False, num_select=0, visualization=False):
        ...
        if visualization == True:
            ...
            for i in range(len(similarities)):
                ...
                print(temp0)  
#             if num_select == 1:
#                 print("'num_select' is set to 1, exit.")
#                 exit()
    ...
    def forward(self, x: torch.Tensor, patches=False, visualization=False):
        ...
        x = torch.cat([self.token_9_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1) 
        x = torch.cat([self.token_10_embedding.to(x.dtype) + torch.zeros(x.shape[0], 1, x.shape[-1], dtype=x.dtype, device=x.device), x], dim=1) 
        
        x = self.ln_pre(x) # (100, 5 + 1 + 49, 768)
        
        x = self.transformer_1(x) # go through 8 transformer layers
        
        x = self.select(x, visualization=visualization, num_tokens=num_tokens)
        
        x = self.transformer_2(x)
        
        x = self.select(x, select=True, num_select=1, visualization=visualization, num_tokens=num_tokens)
        ...
        x_s5 = self.avg(x_s5.permute(0,2,1)).squeeze()
        ...
        x_s5 = x_s5 @ self.proj_s5 # 768 -> 512
        
        if patches == False:
            return x_contra
        else:
            return x_contra, x_s1, x_s2, x_s3, x_s4, x_s5
...
#         self.proj_cls_mid =  nn.Parameter(scale * torch.randn(width, output_dim))       
    ...
    def forward(self, image_input, text_inputs, tags, train=False):
        ...
        t5 = self.encode_text(t5)
        
        # obtain image features
        x_cls, x_s1, x_s2, x_s3, x_s4, x_s5 = self.encode_image(image_input, patches=True)
        ...
        s5_features = logit_scale.sqrt() * x_s5 / x_s5.norm(dim=-1, keepdim=True)
        ...
        t5_f = logit_scale.sqrt() * t5 / t5.norm(dim=-1, keepdim=True)
        
        if train == False:
            return image_features, text_features
        else: 
            return image_features, text_features, s1_features, s2_features, s3_features, s4_features, s5_features, t1_f, t2_f, t3_f, t4_f, t5_f
