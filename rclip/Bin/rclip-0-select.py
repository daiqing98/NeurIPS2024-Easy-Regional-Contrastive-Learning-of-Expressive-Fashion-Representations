...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int):
        ...
        # Vision Transformer
        self.transformer = Transformer(width, 11, heads)
        self.transformer_last = Transformer(width, 1, heads)
    ...
    def forward(self, x: torch.Tensor, patches=False):
        ...
        x = self.transformer(x) # go through 11 transformer layers
        
        x = x.permute(1, 0, 2)  # LND -> NLD, [100, 5 + 1 + 49, 768]
        
        # select most relevant tokens
        x_token_cls = x[:, 5, :] # cls token, (100, 768), global feature
        group_tokens = x[:, 0:5, :]
        patch_tokens = x[:, 6:, :]
        # similarities: 
        q = torch.matmul(group_tokens, self.proj_q) # 100, 10, 768 @ 768, 768
        k = torch.matmul(patch_tokens, self.proj_k) # 100, 49, 768 @ 768, 768
        v = torch.matmul(patch_tokens, self.proj_v) # 100, 49, 768 @ 768, 768                     
...
#         # visualization
#         temp0 = torch.argmax(similarities[0].squeeze(), dim=1)
#         print(temp0)
#         temp1 = torch.argmax(similarities[1].squeeze(), dim=1)
#         print(temp1)
        
#         exit()

        result = torch.bmm(similarities, v) # (100, 5, 49) @ (100, 49, 768) = (100, 5, 768), selected 5 tokens
    
        # ----- 0-select -----
        result = v[:,0,:].unsqueeze(1)
        # ----- 0-select -----
    
        x_selected_pool = self.avg(result.permute(0,2,1)).squeeze() # (100, 768, 5) -> (100, 768)
        
        x_selected_pool = self.ln_selection(x_selected_pool)
        x = torch.cat((x_token_cls.unsqueeze(1), result), dim=1) # (100, 1+5=6, 768) # only keep this for final version
 
        
        x = self.ln_mid(x) # Layer Norm for x
        x = x.permute(1, 0, 2) # NLD -> LND
        x = self.transformer_last(x)
        x = x.permute(1, 0, 2)  # LND -> NLD
        
        x_contra_g = x[:, 0, :] # cls token, (100, 768), global feature
        x_contra_g = self.ln_post_cls(x_contra_g)
        
        # Proj        
        x_contra_g = x_contra_g @ self.proj_cls # 768 -> 512
        x_selected_pool = x_selected_pool @ self.proj_cls_mid # 768 -> 512
        
        if patches == False:
            return x_contra_g
        else:
            return x_contra_g, x_selected_pool
...
#         self.proj_cls_mid =  nn.Parameter(scale * torch.randn(width, output_dim))       
    ...
    def encode_image(self, image, patches=False):
        return self.visual.forward(image.type(self.dtype), patches)
    ...
    def forward(self, image_input, text_inputs, train=False):
        
        text_features = self.encode_text(text_inputs) # text_features: (100, 512)
        # obtain image features
        x_cls, x_selection = self.encode_image(image_input, patches=True)
        ...
        # normalized features
        image_features =  logit_scale * image_features / image_features.norm(dim=-1, keepdim=True)
        text_features = logit_scale * text_features / text_features.norm(dim=-1, keepdim=True)
        image_features_selected = logit_scale * x_selection / x_selection.norm(dim=-1, keepdim=True)
        
        if train == False:
            return image_features, text_features
        else: 
            return image_features, text_features, image_features_selected
