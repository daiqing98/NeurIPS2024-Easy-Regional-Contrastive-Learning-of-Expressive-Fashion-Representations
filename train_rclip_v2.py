    ...
    for epoch in range(args.epochs):
        ...
        for step, (image_input, texts, text_inputs_raw, (comp_str, category_str, brand_str, season_str, text)) in enumerate(dataloader):
            ...
            with autocast(enabled=args.fp16):
                image_features, text_features, s1_f, s2_f, s3_f, s4_f, t1_f, t2_f, t3_f, t4_f = model(image_input, text_inputs, (t1,t2,t3,t4), train = True)
                ...
                loss = loss_contra * 0.5 +  (loss_s1 + loss_s2 + loss_s3 + loss_s4)/4 * 0.5
...
#     model_clip.load_state_dict(torch.load('../CLIP/models/clip_ft/clip_ft_E10'))
    ...
    from rclip.rclip_v2 import rCLIP
    ...
    if args.train_select == False:
        state_dict = torch.load('./OUTPUT/OUTPUT-rCLIP_v2-select-shu/select_5_5e-4.pth')
...
#     model_rclip.load_state_dict(state_dict['state_dict'])
    ...
    train_data = FashionGen(data_root=args.data_root, mode='train')
    val_data = FashionGen(data_root=args.data_root, mode='validation')
